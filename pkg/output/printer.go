...
// Paging enables or disables paging of results. If enabled the and the command line `less` tool is
// available, it will be used to display the output page by page. The default value is true, so
...
func (b *PrinterBuilder) Build(ctx context.Context) (result *Printer, err error) {
	...
	pagerPath, err := pagerPath()
	if err != nil {
		return
	}

	// Check if the output is a TTY:
	isTTY, err := isTTY(b.writer)
	if err != nil {
		return
	}
...
func isTTY(writer io.Writer) (result bool, err error) {
...
func pagerPath() (result string, err error) {
	result, err = exec.LookPath("less")
