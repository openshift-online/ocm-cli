...
func updateWorkloadIdentityConfigurationCmd(cmd *cobra.Command, argv []string) error {
	...
	if UpdateWifConfigOpts.FederatedProject == "" {
		if wifConfig.Gcp().FederatedProjectId() == "" {
			UpdateWifConfigOpts.FederatedProject = wifConfig.Gcp().ProjectId()
		} else {
			UpdateWifConfigOpts.FederatedProject = wifConfig.Gcp().FederatedProjectId()
		}
	}
	...
	projectNum, err := gcpClient.ProjectNumberFromId(ctx, UpdateWifConfigOpts.FederatedProject)
	if err != nil {
	...
	wifBuilder.Gcp(cmv1.NewWifGcp().
		...
		FederatedProjectNumber(strconv.FormatInt(projectNum, 10)))
	...
	if UpdateWifConfigOpts.Mode == ModeManual {
		...
		projectNumInt64, err := strconv.ParseInt(wifConfig.Gcp().FederatedProjectNumber(), 10, 64)
		if err != nil {
