...
// run is the Cobra RunE handler for "ocm create kubeletconfig".
func run(cmd *cobra.Command, argv []string) error {
	...
	if existingResponse != nil && existingResponse.Status() != http.StatusNotFound {
		return fmt.Errorf(
			"A KubeletConfig for cluster '%s' already exists. "+
				"You should edit it via 'ocm edit kubeletconfig'",
			clusterKey,
		)
	}
	...
	if args.yes {
		...
		confirmed, err = kc.ConfirmWorkerNodeReboot("Creating")
		if err != nil {
			return err
		}
	}
